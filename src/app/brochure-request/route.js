...
import { BrochureSchema } from "@/serverless/validators";
...
import { buildBrochureMail } from "@/serverless/builders";
...
export async function POST(req) {
  try {
    ...
    const res = BrochureSchema.safeParse(body);
    if (!res.success) {
    ...
    const rl = await rateLimit("brochure-request", ip);
    if (!rl.ok) {
      const h = securityHeaders(origin);
      ...
      h.set("X-Request-Id", reqId);
      logInfo("rate_limited", { route: "/brochure-request", ip, reqId });
      return NextResponse.json({ error: "Too many requests" }, { status: 429, headers: h });
    }

    const mailOptions = buildBrochureMail(res.data);
    await sendMail(mailOptions);

    const h = securityHeaders(origin);
    // Grant short-lived brochure download after successful submission
    try {
      const grant = createGrant("brochure", 180);
      h.append?.("Set-Cookie", buildDownloadCookie("svs_dl_brochure", grant, 180));
    } catch {}
    const obj = rateLimitHeaders(rl);
    for (const k in obj) h.set(k, obj[k]);
    h.set("X-Request-Id", reqId);
    logInfo("brochure_sent", { route: "/brochure-request", ip, reqId });
    return NextResponse.json({ message: "Brochure email sent successfully" }, { headers: h });
  } catch (err) {
    console.error("/brochure-request error:", err?.message || err);
    const h = securityHeaders(null);
    h.set("X-Request-Id", getRequestId(req.headers));
    logError("brochure_error", { route: "/brochure-request", error: err?.message || String(err) });
    return NextResponse.json({ error: "Failed to send brochure email" }, { status: 500, headers: h });
  }
}
